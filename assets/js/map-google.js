$(document).ready(function($) {
    ...
    if( $("#"+mapId).length ) {
        ...
        map = new google.maps.Map(document.getElementById("ts-map-hero"), {
            ...
            // center: new google.maps.LatLng(25.0573368, 55.1315423),
            center: new google.maps.LatLng(centerLatitude, centerLongitude),

            mapTypeId: "roadmap",

            clickableIcons: false, // this is to disable all labels icons except yo
    ...
    */
    ...
    function createMarkers(){
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            '<a href="#" class="ts-marker" data-ts-id="'+ loadedMarkersData[i]["id"] +'">' +
                ...
                ( ( loadedMarkersData[i]["BuildingName"] !== undefined ) ? '<div class="ts-marker__title">'+ loadedMarkersData[i]["BuildingName"] +'</div>' : "" ) +
                ...
                ( ( loadedMarkersData[i]["NoOfOfficesRent"] !== undefined && loadedMarkersData[i]["NoOfOfficesRent"] > 0 ) ? '<div class="ts-marker__info  rent-info">' + (loadedMarkersData[i]["NoOfOfficesRent"]) + ' Offices for rent' +'</div>' : "" ) +

            ((loadedMarkersData[i]["NoOfOfficesSale"] !== undefined && loadedMarkersData[i]["NoOfOfficesSale"] > 0) ? '<div class="ts-marker__info orange">' + (loadedMarkersData[i]["NoOfOfficesSale"]) + ' Offices for sale' + '</div>' : "") +

            ((loadedMarkersData[i]["NoOfBusinessCenter"] !== undefined && loadedMarkersData[i]["NoOfBusinessCenter"] > 0) ? '<div class="ts-marker__info  rent-info">' + (loadedMarkersData[i]["NoOfBusinessCenter"]) + ' Business Center' + '</div>' : "") +

                ( ( loadedMarkersData[i]["SmallImage"] !== undefined ) ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url('+ loadedMarkersData[i]["SmallImage"] +')"></div>' : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(/assets/img/marker-default-img.png)"></div>' ) +
            '</a>';
    ...
    function markersDone(){
        ...
        markerCluster = new MarkerClusterer(map, newMarkers, { styles: clusterStyles, ignoreHidden: true });
    ...
    function createInfoBoxHTML(parameters){
        ...
        */
        ...
        '<div class="ts-infobox" data-ts-id="'+ loadedMarkersData[i]["id"] +'">' +
            ...
            ( ( loadedMarkersData[i]["Ribbon"] !== undefined && loadedMarkersData[i]["Ribbon"] > '' ) ? '<div class="ts-ribbon">'+ loadedMarkersData[i]["Ribbon"] +'</div>' : "" ) +
            ( ( loadedMarkersData[i]["ribbon_corner"] !== undefined ) ? '<div class="ts-ribbon-corner"><span>'+ loadedMarkersData[i]["ribbon_corner"] +'</span></div>' : "" ) +

            '<a href="'+ loadedMarkersData[i]["DetailsLink"] +'" class="ts-infobox__wrapper ts-black-gradient">' +
                ...
                '<div class="ts-infobox__content">' +
                    '<figure class="ts-item__info">' +
                        ...
                        ( ( loadedMarkersData[i]["NoOfOfficesRent"] !== undefined && loadedMarkersData[i]["NoOfOfficesRent"] > 0 ) ? '<div class="ts-item__info-badge green">' + (loadedMarkersData[i]["NoOfOfficesRent"])+ ' Offices for rent' +'</div>' : "" ) +
                        ((loadedMarkersData[i]["NoOfOfficesSale"] !== undefined && loadedMarkersData[i]["NoOfOfficesSale"] > 0) ? '<div class="ts-item__info-badge ">' + (loadedMarkersData[i]["NoOfOfficesSale"]) + ' Offices for sale' + '</div>' : "") +
                        ((loadedMarkersData[i]["NoOfBusinessCenter"] !== undefined && loadedMarkersData[i]["NoOfBusinessCenter"] > 0) ? '<div class="ts-marker__info  rent-info">' + (loadedMarkersData[i]["NoOfBusinessCenter"]) + ' Business Center' + '</div>' : "") +

                        ( ( loadedMarkersData[i]["BuildingName"] !== undefined && loadedMarkersData[i]["BuildingName"].length > 0 ) ? '<h4>'+ loadedMarkersData[i]["BuildingName"] +'</h4>' : "" ) +
                        ( ( loadedMarkersData[i]["LicenseName"] !== undefined && loadedMarkersData[i]["LicenseName"].length > 0 ) ? '<aside> Licence Type : '+ loadedMarkersData[i]["LicenseName"] +'</aside>' : "" ) +
                        ( ( loadedMarkersData[i]["Status"] !== undefined && loadedMarkersData[i]["Status"].length > 0 ) ? '<aside> Status  : '+ loadedMarkersData[i]["Status"] +'</aside>' : "" ) +

                        ( ( loadedMarkersData[i]["Location"] !== undefined && loadedMarkersData[i]["Location"].length > 0 ) ? '<aside> Area  : '+ loadedMarkersData[i]["Location"] +'</aside>' : "" ) +

                        ( ( loadedMarkersData[i]["BuildingClass"] !== undefined && loadedMarkersData[i]["BuildingClass"].length > 0 ) ? '<aside> Building Class : '+ loadedMarkersData[i]["BuildingClass"] +'</aside>' : "" ) +

                        ( ( loadedMarkersData[i]["Category"] !== undefined && loadedMarkersData[i]["Category"].length > 0 ) ? '<aside> Usage : '+ loadedMarkersData[i]["Category"] +'</aside>' : "" ) +
                ...
                '</div>' +
                '<div class="ts-infobox_image" style="background-image: url('+ loadedMarkersData[i]["SmallImage"] +')"></div>' +
            '</a>' +
        '</div>';
    }
    ...
    $('.click').on('mouseover', function () {
        // map.LatLng(3.165559, 101.612416);
        var lath = $(this).attr("data-ts-lat");
        var langh = $(this).attr("data-ts-lang");
        var myLatlng = new google.maps.LatLng(lath,langh);
    ...
    $('.click').on('click', function () {
        ...
        var buttonId = $(this).parent(".ts-result-link").attr("data-ts-id");
        // var TargetInfoBox = $(".ts-infobox").attr("data-ts-id");
        var ParentM = $(".ts-marker[data-ts-id=" + buttonId + "]");
        var ParentMClick = $(ParentM).parent().parent();
        // console.log().length;
        setTimeout(function(){ParentMClick.trigger("click"); }, 500);

        // $(document).on('DOMNodeInserted', function(e) {
        //     if ( $(ParentMClick).length == 1 ) {
